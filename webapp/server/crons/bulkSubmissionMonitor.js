...
const bulkSubmissionMonitor = async () => {
  try {
    ...
    // parse conf.json
    const bulkSubmissionHome = path.join(config.PROJECTS.BULK_DIR, bulkSubmission.code);
    ...
    rows.forEach(async cols => {
      ...
      if (cols[4] && cols[4].trim()) {
        ...
        if (submission.platform === 'PacBio') {
          ...
          const pairFq1 = [];
          const pairFq1_display = [];
          const pairFq2 = [];
          const pairFq2_display = [];
          let fq1s = null;
          let fq2s = null;
          ...
          if (validInput) {
            ...
            for (let i = 0; i < pairFq1.length; i++) {
              submission.input_fastqs.push({ 'fq1': pairFq1[i], 'fq2': pairFq2[i] });
              submission.input_fastqsDisplay.push({ 'fq1': pairFq1_display[i], 'fq2': pairFq2_display[i] });
            }
          }
        }
      }
