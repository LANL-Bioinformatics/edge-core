...
const getOne = async (req, res) => {
  try {
    ...
    const bulkSubmission = await getBulkSubmission(req.params.code, 'user', req.user);
...
const getConf = async (req, res) => {
  try {
    ...
    logger.error(`/api/auth-user/bulkSubmissions/${req.params.code}/conf failed: ${err}`);
    return res.status(500).json({
      message: sysError,
      success: false,
    });
...
const getOwn = async (req, res) => {
  try {
    ...
    const bulkSubmissions = await BulkSubmission.find({ 'type': { $ne: 'sra2fastq' }, 'status': { $ne: 'delete' }, 'owner': { $eq: req.user.email } }).sort([['updated', -1]]);
...
const getProjects = async (req, res) => {
  try {
    ...
    logger.error(`/api/auth-user/bulkSubmissions/${req.params.code}/projects failed: ${err}`);
    return res.status(500).json({
      message: sysError,
      success: false,
    });
