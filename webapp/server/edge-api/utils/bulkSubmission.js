...
const getBulkSubmission = async (code, type, user) => {
  try {
    ...
    const bulkSubmission = await BulkSubmission.findOne({ 'status': { $ne: 'delete' }, 'code': { $eq: code } });
    ...
    if (type === 'user' && (bulkSubmission.owner === user.email || bulkSubmission.sharedTo.includes(user.email) || bulkSubmission.public)) {
      return bulkSubmission;
    }
